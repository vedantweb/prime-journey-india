      ...
      <div className="mx-auto max-w-7xl px-5 sm:px-8">
        <div className="flex flex-col gap-6 md:flex-row md:items-end md:justify-between">
          <SectionHeading
            ...
            sub="Not just places — the hours inside them that stay with you. Tap any experience to start planning."
          />
          <Reveal delay={0.2} className="shrink-0">
            <p className="font-editorial text-lg italic text-gold/90">Collect moments, not checklists.</p>
          </Reveal>
        </div>
      </div>
        ...
        <div data-testid="experiences-track" className="no-scrollbar mt-14 flex snap-x snap-mandatory gap-6 overflow-x-auto px-5 pb-4 sm:px-8 lg:px-[max(2rem,calc((100vw-80rem)/2+2rem))]">
          {experiences.map((e, i) => (
            ...
            >
              ...
              <span className="absolute left-5 top-5 font-display text-5xl font-extrabold text-white/25 transition-colors duration-500 group-hover:text-gold/70">
                {String(i + 1).padStart(2, '0')}
              </span>
              <div className="absolute inset-x-0 bottom-0 p-6">
                <p className="text-[11px] font-bold uppercase tracking-[0.3em] text-gold">{e.place}</p>
                <h3 className="mt-1.5 font-display text-2xl font-extrabold">{e.title}</h3>
